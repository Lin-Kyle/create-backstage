...
const getIcon = icon => {
    if (typeof icon === 'string' && icon.indexOf('http') === 0) {
        return <img src={icon} alt="icon" className={` sider-menu-item-img`} style={{
                width: '14px',
                marginRight: '10px'
            }}/>;
    }
...
@withRouter
@inject(allStore => ({global: allStore.global}))@observer
export default class SiderMenu extends Component {
    ...
    getSubMenuOrItem = item => {
        ...
        if (item.children && item.children.some(child => child.name)) {
            ...
            if (childrenItems && childrenItems.length > 0) {
                return (<SubMenu data-api={JSON.stringify(item.api)} title={item.icon
                        ? (<span>
                            {getIcon(item.icon)}
                            <span>{item.name}</span>
                        </span>)
    ...
    getMenuItemPath = item => {
        ...
        return (<Link to={itemPath} target={target}
    ...
    render() {
        ...
        return (<Sider trigger={null} collapsible="collapsible" breakpoint="lg" width={200} className={'sider'} collapsed={global.collapsed}>
            ...
            <Menu onClick={this.changeCurrentMenuData} key='menu' theme="dark" mode="inline" selectedKeys={[this.getSelectedKeys()]} defaultOpenKeys={[this.getDefaultOpenKey()]}>
